  ...
  const CreateTicket = async (e) => {
    ...
    try {
      ...
      const resTicket = await fetch(
        `http://ticket-env.eba-3gvvmzhz.us-east-1.elasticbeanstalk.com/tickets`,
        {
          method: 'POST',
          headers: {
          ...
          body: JSON.stringify({ titulo, descripcion, id_categoria: categoria, id_prioridad: prioridad}),
        }
      );
      ...
      if (comentarios !== '') {
        ...
        const resComentario = await fetch(
          `http://ticket-env.eba-3gvvmzhz.us-east-1.elasticbeanstalk.com/tickets/${ticketData.ticket_id}/comments`,
          {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
        ...
        if (!resComentario.ok) throw new Error('Error al agregar el comentario');
